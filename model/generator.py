...
import model.transformer_utils as transformer_utils
...
class ResidualBlockNoBN(nn.Module):
    ...
    def forward(self, x):
        identity = x
        out = self.conv2(self.dropout(self.lrelu(self.conv1(x))))
...
class make_dilation_dense(nn.Module):
    ...
    def forward(self, x):
        out = self.dropout(self.lrelu(self.conv(x)))
...
class AttentionModule(nn.Module):
    ...
    def __init__(self, nf=64):
        ...
        self.attn_blks = nn.Sequential(transformer_utils.TransformerBlock_3(dim=nf, num_heads=1, ffn_expansion_factor=2.66, bias=True),
                                       transformer_utils.TransformerBlock_3(dim=nf, num_heads=1, ffn_expansion_factor=2.66, bias=True),
                                       transformer_utils.TransformerBlock_3(dim=nf, num_heads=1, ffn_expansion_factor=2.66, bias=True))
...
class Downsample_block_normal(nn.Module):
...
class Upsample_block_normal(nn.Module):
...
class Generator(nn.Module):
    ...
    def __init__(self, nChannel, nDenselayer, nFeat, growthRate):
        ...
        self.attn_layer = nn.Sequential(transformer_utils.TransformerBlock_4(dim=256, num_heads=1, ffn_expansion_factor=2.66, bias=True),
                                        transformer_utils.TransformerBlock_4(dim=256, num_heads=1, ffn_expansion_factor=2.66, bias=True),
                                        transformer_utils.TransformerBlock_4(dim=256, num_heads=1, ffn_expansion_factor=2.66, bias=True))
    ...
    def forward(self, x):
        ...
        attn_entropy = 0
        attn_outs = self.attn_layer([feat, s_feat, m_feat, l_feat, attn_entropy])
        feat = self.upsample_block[0](attn_outs[0])
        feat = self.upsample_block[1](feat)
        feat = coarse_hdr_feat + feat
        res = self.post_conv(feat)

        attn_entropy = attn_outs[4] / 3.

        return res, attn_entropy
