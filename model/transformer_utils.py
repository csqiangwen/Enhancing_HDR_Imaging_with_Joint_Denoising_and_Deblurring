...
class Attention_4(nn.Module):
    ...
    def forward(self, x):
        ...
        kv_input = torch.stack(x[1:], dim=1) # stack [_1_feat, _2_feat, _3_feat], shape is [b, ref_num, c, h, w]
        q = self.q_conv(q_input).unsqueeze(1) # shape is [b, 1, c, h, w]
        k = self.k_conv(kv_input.view(-1, *kv_input.shape[2:])).view(kv_input.shape) # shape is [b, ref_num, c, h, w]
        v = self.v_conv(kv_input.view(-1, *kv_input.shape[2:])).view(kv_input.shape) # shape is [b, ref_num, c, h, w]
        ...
        attn_entropy = torch.mean(torch.sum(attn_entropy, dim=-1))
...
class TransformerBlock_4(nn.Module):
    ...
    def forward(self, x):
        [_0_feat, _1_feat, _2_feat, _3_feat, attn_entropy] = x
        residual_feat, attn_entropy_new = self.attn([self.norm1(_0_feat), self.norm1(_1_feat), self.norm1(_2_feat), self.norm1(_3_feat)])
        _0_feat = _0_feat + residual_feat
        _0_feat = _0_feat + self.ffn(self.norm2(_0_feat))

        attn_entropy += attn_entropy_new
        return [_0_feat, _1_feat, _2_feat, _3_feat, attn_entropy]
