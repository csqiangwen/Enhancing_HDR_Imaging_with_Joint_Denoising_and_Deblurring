...
class HDRDataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.folder_path = root_path / 'test_tmp' / 'static_new'
    ...
    def __getitem__(self, index):
        ...
        # Read images
        [s_LDR, m_LDR, l_LDR, GT_HDR] = [cv2.imread(str(img_path / 'gt' / 'short.tif')),
        ...
        [s_LDR, m_LDR, l_LDR, GT_HDR] = [torch.from_numpy(s_LDR.transpose(2,0,1).astype(np.float32)) * 2. - 1.,
                                         torch.from_numpy(m_LDR.transpose(2,0,1).astype(np.float32)) * 2. - 1.,
                                         torch.from_numpy(l_LDR.transpose(2,0,1).astype(np.float32)) * 2. - 1.,
                                         torch.from_numpy(GT_HDR.transpose(2,0,1).astype(np.float32)) * 2. - 1.]
